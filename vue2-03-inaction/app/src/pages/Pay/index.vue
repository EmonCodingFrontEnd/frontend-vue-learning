  ...
  methods: {
    ...
    async open() {
      const url = await QRCode.toDataURL(
        this.payInfo.codeUrl || "已支付的订单没有codeUrl"
      );
      this.$alert(`<img src="${url}"/>`, "标题名称", {
        ...
        beforeClose: (action, instance, done) => {
          ...
          // http://localhost:8080/#/pay?orderId=832  已支付
          ...
          // http://localhost:8080/#/pay?orderId=1025 未支付
        ...
        .then((value) => {})
        .catch((reason) => {}); // 点击取消按钮，会执行这个方法，若不配置，会报错！！！
      if (!this.timer) {
