      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con6">
            <span class="sum">{{ subtotal(cart) }}</span>
          </li>
...
export default {
  ...
  methods: {
    ...
    // 单个sku总价小计
    subtotal(cart) {
      return (cart.skuNum * cart.skuPrice).toFixed(2);
    },
    // 修改某一个产品的个数
    changeSkuNum: debounce(async function (type, disNum, cart) {
      switch (type) {
        case "minus":
    ...
    // 修改某一个产品的勾选操作
    updateChecked: throttle(async function (cart, event) {
      try {
  ...
  computed: {
    ...
    totalPrice() {
      ...
      // });
      sum = this.cartInfoList.reduce(
        (pre, cur) => pre + cur.skuNum * cur.skuPrice,
        0
      );
      return sum;
    },
